...
#
# Options:
#   --hostname <fqdn>   mail host FQDN (EHLO/cert name; also derives SPF/DMARC).
#                       Default: this machine's `hostname -f`.
#   --tls               production TLS: Let's Encrypt on :443 + mail ports :587/:465/:25.
#   --acme-email <addr> contact email for Let's Encrypt (with --tls).
...
#
# The SPF include target, DMARC address, and sending IPv4/IPv6 are derived from
# the hostname and the host's public IPs automatically. On a NAT'd host only the
# private IP is visible, so pass --public-ipv4/--public-ipv6 (or set them in
# relay.toml). Override spf_include / dmarc_rua likewise if needed.
#
# Also overridable via environment (or an existing relay.toml - that always wins):
#   RELAY_HOSTNAME · DB_NAME / DB_USER (default relay) · DB_PASSWORD (generated)
#
set -euo pipefail
...
# Resolve hostname + listen addr + TLS from flags/env (defaults for dev).
HOST="${HOSTNAME_ARG:-${RELAY_HOSTNAME:-$(hostname -f 2>/dev/null || hostname)}}"
...
# SPF include, DMARC address, and sending IPs are auto-derived from the hostname
# and the host's public IPs; set spf_include / dmarc_rua / sending_ipv4 /
# sending_ipv6 here to override.
hostname         = "$HOST"
http_addr        = "$HTTP_ADDR"
submission_addr  = ":587"
...
acme_http_addr = ":80"
TOML
  # Pin the public sending IPs when supplied (required for NAT/port-forwarded hosts).
  [[ -n "$PUB_V4" ]] && printf 'sending_ipv4 = "%s"\n' "$PUB_V4" >> relay.toml
  [[ -n "$PUB_V6" ]] && printf 'sending_ipv6 = "%s"\n' "$PUB_V6" >> relay.toml
  umask 022
  chmod 600 relay.toml
  ok "generated relay.toml (0600) for ${B}$HOST${N} (TLS=$TLS_ENABLED) with fresh secrets"
fi
